...
document.addEventListener("DOMContentLoaded", () => {
  // Initialize EmailJS directly (Note: Replace with real public key to use)
  if (typeof emailjs !== 'undefined') {
    emailjs.init("8GkF5x2WfwegqkEOQ"); 
  }
  ...
  // Start the background checker for scheduled tasks
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    if (timeLeft > 0) {
      ...
      // Trigger a "Near Completion" reminder at exactly 5 minutes (300 seconds) left
      if (timeLeft === 300) {
        sendWarningEmail(focusedTaskId);
      }
    } else {
...
function sendWarningEmail(taskId) {
  const userEmail = localStorage.getItem("taskmate_user_email");
  const taskTitle = document.getElementById("focusTaskTitle").innerText;
  const emailMessage = `Just a quick reminder: You have exactly 5 minutes left on your focus session for "${taskTitle || "Focus Session"}". Keep going strong!`;

  if (userEmail && typeof emailjs !== 'undefined') {
    emailjs.send("service_z7nbyu8", "template_w3yvhh8", {
      to_email: userEmail,
      task_title: taskTitle || "Focus Session",
      message: emailMessage,
    }).then(() => console.log('5-Minute Reminder Email sent successfully via EmailJS!'))
      .catch((err) => console.log('EmailJS Reminder failed: ', err));
  }
}
...
function stopTimer(message) {
  ...
  if (message) {
    ...
    // EmailJS Notification (Direct Backendless Email)
    const userEmail = localStorage.getItem("taskmate_user_email");
    const taskTitle = document.getElementById("focusTaskTitle").innerText;
    
    // Find next pending task to remind them about
    const nextTask = tasks.find(t => t.status !== "done" && t.id !== focusedTaskId);
    const nextTaskMessage = nextTask ? `Your next task is: ${nextTask.title}.` : "You have no more pending tasks!";
    const emailMessage = `You've successfully completed a focus session for: ${taskTitle || "Focus Session"}. ${nextTaskMessage}`;

    if (userEmail && typeof emailjs !== 'undefined') {
      console.log("Triggering EmailJS to " + userEmail);
      // Replace these IDs with actual EmailJS service/template IDs
      emailjs.send("service_z7nbyu8", "template_w3yvhh8", {
        to_email: userEmail,
        task_title: taskTitle || "Focus Session",
        message: emailMessage,
      }).then(() => console.log('Email sent successfully via EmailJS!'))
        .catch((err) => console.log('EmailJS setup incomplete/placeholder used: ', err));
    }
...
function setupEventListeners() {
  ...
  taskForm.addEventListener("submit", (e) => {
    ...
    const newTask = {
      ...
      dueTime: document.getElementById("dueTime") ? document.getElementById("dueTime").value : "",
      status: "todo",
  ...
  // 8. User Email Preferences
  const emailInput = document.getElementById("userEmailInput");
  if (emailInput) {
    emailInput.value = localStorage.getItem("taskmate_user_email") || "";
    emailInput.addEventListener("blur", () => {
      localStorage.setItem("taskmate_user_email", emailInput.value);
...
window.fillEditForm = (id) => {
  ...
  if (document.getElementById("dueTime")) document.getElementById("dueTime").value = task.dueTime || "";
...
function startScheduledTaskChecker() {
  ...
  setInterval(() => {
    ...
    tasks.forEach(task => {
      // Skip if done, no due date, no due time, or reminder already sent
      if (task.status === "done" || !task.dueDate || !task.dueTime || task.reminderSent) return;

      // Create a Date object for the task's exact due deadline
      ...
      // If the task is due in exactly 5 minutes (or within the 5 minute window for the first time)
      if (diffMins > 0 && diffMins <= 5) {
        // Send Email
        const emailMessage = `Reminder: Your scheduled task "${task.title}" is due in ${diffMins} minutes!`;
        ...
        emailjs.send("service_z7nbyu8", "template_w3yvhh8", {
          ...
          console.log(`Scheduled reminder sent for: ${task.title}`);
          // Mark as sent so we don't spam them every 30 seconds
          task.reminderSent = true;
          saveTasks(); 
        }).catch((err) => console.log('Scheduled Reminder failed: ', err));
      }
    });
  }, 30000); 
}
